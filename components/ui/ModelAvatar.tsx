...
import { Wifi, WifiOff, Activity, CheckCircle, AlertTriangle, Cpu, Sparkles, Box, Zap } from 'lucide-react';
...
const GeminiLogo = ({ color }: { color: string }) => (
...
const OpenAILogo = ({ color }: { color: string }) => (
...
const DeepSeekLogo = ({ color }: { color: string }) => (
  <svg viewBox="0 0 24 24" fill={color} xmlns="http://www.w3.org/2000/svg" className="w-full h-full p-1">
    {/* Simplified Whale/Curve Shape */}
...
const ModelAvatar: React.FC<ModelAvatarProps> = ({ providerId, name, color, status, mini = false }) => {
  const getStatusIcon = () => {
    switch (status) {
  ...
  const renderLogo = () => {
    switch (providerId) {
      ...
      default: return <Cpu size={mini ? 20 : 24} style={{ color }} />;
    }
  };
  ...
  return (
    <div className="relative inline-block">
      <div 
        className={`${sizeClass} rounded-sm border-2 bg-black flex items-center justify-center transition-all duration-300 relative overflow-hidden`}
        style={glowStyle}
      >
        {/* Brand Logo */}
